...
@section('content')
                ...
                <form method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="cutting_place"
                            placeholder="Masukkan nama tempat" value="{{ $slaughteringPlace->cutting_place }}" required>
                        @if ($errors->has('name'))
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="address" name="address"
                            placeholder="Masukkan alamat" value="{{ $slaughteringPlace->address }}" required>
                        ...
                        <select class="form-select" id="type_of_place" name="type_of_place_id" required>
                            ...
                            @foreach ($typeOfPlace as $type)
                                <option value="{{ $type->id }}"
                                    {{ $slaughteringPlace->type_of_place_id == $type->id ? 'selected' : '' }}>
                                    {{ $type->type_of_place }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="kecamatan" name="kecamatan_id" required>
                            ...
                            @foreach ($kecamatan as $id => $nama)
                                <option value="{{ $id }}"
                                    {{ $slaughteringPlace->kecamatan_id == $id ? 'selected' : '' }}>
                                    {{ $nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-select" id="kelurahan" name="kelurahan_id" required>
                            ...
                            @foreach ($kelurahan as $kel)
                                <option value="{{ $kel->id }}" data-kecamatan-id="{{ $kel->kecamatan_id }}"
                                    {{ $slaughteringPlace->kelurahan_id == $kel->id ? 'selected' : '' }}>
                                    {{ $kel->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <input type="hidden" id="latitude" name="latitude" value="{{ $slaughteringPlace->latitude }}"
                        required>
                    <input type="hidden" id="longitude" name="longitude" value="{{ $slaughteringPlace->longitude }}"
                        required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const map = L.map('map').setView([{{ $slaughteringPlace->latitude }},
                {{ $slaughteringPlace->longitude }}
            ], 13);
